...
PREEXISTING_REPAIRS = {
    "Wondrous Item, Legendary (Requires Attunement)": {
        ...
        "evidence": "Bigby page 116: DELVER'S CLAWS is paired with Wondrous Item, Rare (Requires Attunement).",
    },
    "ARMOR OF": {
        "name": "ARMOR, +1, +2, OR +3",
        ...
        "evidence": "2014 DMG page 153: split heading ARMOR, +1, +2, OR +3 precedes the light/medium/heavy rarity descriptor.",
    },
    "CHAP TER 7 TREASURE": {
        ...
        "rarity": "Uncommon",
        "requires_attunement": 1,
        "evidence": "2014 DMG page 201: CHAPTER heading was captured instead of SLIPPERS OF SPIDER CLIMBING.",
    },
    "Wondrous Item, Uncommon": {
        ...
        "rarity": "Uncommon",
        "requires_attunement": 0,
        "evidence": "2024 DMG context describes imperceptible gloves granting +5 Dexterity (Sleight of Hand): GLOVES OF THIEVERY.",
    },
    "Wondrous Item, Common": {
        ...
        "max_charges": 3,
        "recharge_kind": "DAWN",
        "evidence": "2024 DMG page 271: HAT OF VERMIN heading directly precedes Wondrous Item, Common.",
    },
    "Mace oF DisruPTION Weapon (Mace), Rare (Requires Attunement) ‘When yo…": {
        "name": "MACE OF DISRUPTION",
        "item_type": "Weapon (Mace)",
        "rarity": "Rare",
        "requires_attunement": 1,
        "save_dc": 15,
        "evidence": "2024 DMG page 280: title and descriptor/prose were concatenated; canonical title is MACE OF DISRUPTION.",
    },
    "Wand, Legendary (Requires Attunement)": {
        "name": "STARSHOT CROSSBOW",
        "item_type": "Weapon (Any Crossbow)",
        "rarity": "Rare",
        "requires_attunement": 1,
        "evidence": "Book of Many Things page 39: two-column OCR interleaved SPINDLE OF FATE and STARSHOT CROSSBOW; stored profile is the crossbow descriptor.",
    },
    "Weapon (Any Ammunition), Uncommon": {
        ...
        "rarity": "Rare",
        "requires_attunement": 0,
        "evidence": "Book of Many Things page 68: two-column OCR interleaved DRIED LEECH and GRASPING WHIP; stored profile is the whip descriptor.",
    },
    "OF VERMIN": {
        "name": "HAT OF VERMIN",
        "item_type": "Wondrous Item",
        ...
        "max_charges": 3,
        "recharge_kind": "DAWN",
        "evidence": "Xanathar page 138: split lines HAT / OF VERMIN form HAT OF VERMIN.",
    },
    "CANE": {
        "name": "VETERAN'S CANE",
        "item_type": "Wondrous Item",
        "rarity": "Common",
        "requires_attunement": 0,
        "evidence": "Xanathar page 140: split lines VETERAN'S / CANE form VETERAN'S CANE.",
    },
}
...
def title_quality(name: str) -> bool:
    ...
    # Reject actual descriptor lines, not legitimate names such as
    # "Armor of ..." / "Ring of ..." / "Staff of ...".
    if re.match(
        r"(?i)^(?:"
        r"wondrous\s+item\s*,|weapon\s*\(|armor\s*\(|"
        r"wand\s*,|ring\s*,|rod\s*,|staff\s*,|scroll\s*,|"
        r"potion\s*,|ammunition\s*,|shield\s*,"
        r")",
        n,
    ):
        return False

    # Descriptor/mechanics accidentally concatenated into a title.
    if re.search(r"(?i)\brequires attunement\b", n):
        return False
    if re.search(r"(?i)\b(?:weapon|armor)\s*\([^)]+\)", n):
        return False
    if "…" in n or "..." in n:
        return False

    # Known broken-heading shapes.
    if re.match(r"(?i)^(?:chapter|chap\s*ter)\b", n):
        return False
    if re.match(r"(?i)^of\b", n):
        return False
    if low in {
        "armor of", "cane", "treasure", "sample poisons",
        "wondrous item", "magic item rarity",
    }:
        return False
...
def main() -> int:
    ...
    # Repair the ten evidence-confirmed pre-existing OCR/title corruptions
    # in memory BEFORE 2024 runtime duplicate detection.
    preexisting_repairs = []
    for row in book_magic_rows:
        repair = PREEXISTING_REPAIRS.get(row["item_name"])
        if repair:
            original_name = row["item_name"]
            row["_original_item_name"] = original_name
            row["_repair_evidence"] = repair["evidence"]
            row["item_name"] = repair["name"]
            row["_repair"] = repair
            preexisting_repairs.append(row)

    if len(preexisting_repairs) != 10:
        found = [r.get("_original_item_name") for r in preexisting_repairs]
        raise RuntimeError(
            f"Expected 10 evidence-confirmed pre-existing repairs, got {len(preexisting_repairs)}: {found}"
        )

    # Identify 2024 OCR duplicates against the authoritative runtime registry
    # AFTER repairs. Count is intentionally dynamic; repaired 2024 titles can
    # reveal additional real SRD duplicates.
    book_runtime_dups = []
    clean_book_rows = []
    for row in book_magic_rows:
    ...
    if args.apply:
        ...
        try:
            ...
            # Persist the ten evidence-confirmed repairs. Keep stable content IDs
            # and provenance; only corrupted title/profile fields are corrected.
            for row in preexisting_repairs:
                repair = row["_repair"]
                book_id = row["book_magic_item_id"]
                ...
                values = [
                    repair["item_type"],
                    repair["rarity"],
                    int(repair["requires_attunement"]),
                ]
                for optional_field in ("max_charges", "recharge_kind", "save_dc"):
                    if optional_field in repair:
                        sets.append(f"{optional_field}=?")
                        values.append(repair[optional_field])
                values.append(book_id)
                db.execute(
                    f"UPDATE book_magic_item_engine_profiles SET {', '.join(sets)} "
                    "WHERE book_magic_item_id=?",
                    values,
                )

            # Remove evidence-confirmed 2024 OCR duplicates of authoritative SRD.
            ...
            # Final dispositions for removed 2024 book/runtime duplicates.
            for item in book_runtime_dups:
                row = item["row"]
                db.execute("""
                ...
                """, (
                    ...
                    row.get("_original_item_name", row["item_name"]),"magic-item",row["rules_version"],
            ...
            # Record the repair audit trail. Rows that were subsequently
            # removed as 2024 runtime duplicates still retain this evidence row
            # under a distinct resolution_scope.
            for row in preexisting_repairs:
                db.execute("""
                    INSERT INTO phase6_final_resolution(
                      entity_version_id,resolution_scope,original_name,category,
                ...
                """, (
                    row["entity_version_id"],"book-registry-repair",
                    row["_original_item_name"],"magic-item",row["rules_version"],
                    row["source_title"],row["source_page"],
                    "evidence-confirmed-title-repair",row["item_name"],
                    "book_magic_item_registry",row["book_magic_item_id"],
                    row["_repair_evidence"],batch_id,
                ))
    ...
    report = {
        ...
        "preexistingRepairsApplied": len(preexisting_repairs),
    ...
    print(f"PREEXISTING_REPAIRS_APPLIED={len(preexisting_repairs)}")
