...
EXPECTED = {
    ...
    "review_context": 10,
    "residual_broad_heading_candidates": 87,
}
...
BROAD_BODY = {
    k: [re.compile(p, re.I) for p in pats]
    for k, pats in BROAD_BODY_PATTERNS.items()
}
...
def broad_section_hits(heading, body):
    result = {}
    heading = heading or ""
    body = body or ""
    for family in BROAD:
        heading_hits = sorted({
            m.group(0)
            for rx in BROAD[family]
            for m in rx.finditer(heading)
        }, key=str.lower)
        body_hits = sorted({
            m.group(0)
            for rx in BROAD_BODY[family]
...
def main():
    ...
    try:
        ...
        for r in db.execute(sql):
            all_current_sections += 1
            broad = broad_section_hits(r["heading"] or "", r["body"] or "")
            if not broad:
                continue
            ...
            rows.append({
                ...
                "broad_section_hits": broad,
                "broad_heading_hits": {
                    family: detail["heading_hits"]
                    for family, detail in broad.items()
                    if detail["heading_hits"]
                },
                "broad_body_hits": {
                    family: detail["body_hits"]
                    for family, detail in broad.items()
                    if detail["body_hits"]
                },
    ...
    actual = {
        "all_current_2024_phb_dmg_sections": all_current_sections,
    ...
    report = {
        "package": "PHASE8_2024_CORE_RULE_FAMILY_AUDIT_V3_FIXED",
        "createdAt": now_iso(),
        "readOnly": True,
    ...
    summary = [
        ...
        f"- All current 2024 PHB/DMG sections examined: {all_current_sections}",
        f"- Phase 8 candidate-pool sections (V2 grammar): {core_count}",
    ...
    print(f"ALL_CURRENT_2024_PHB_DMG_SECTIONS={all_current_sections}")
    print(f"CORE_2024_SECTION_CANDIDATES={core_count}")
