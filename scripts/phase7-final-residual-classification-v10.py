...
AOE_HINT_RE = re.compile(
    r"\b(?:cone|line|radius|sphere|cylinder|cube|within\s+\d+\s+(?:feet|ft\.?)|"
...
def apply_v10(db, output):
    ...
    if len(aoe_positive_ids) != 69:
        raise RuntimeError(f"V10 AoE evidence drift: expected=69 actual={len(aoe_positive_ids)}")
    ...
    report = {
        "package": "PHASE7_FINAL_RESIDUAL_CLASSIFICATION_V10",
        "structured": structured,
